...
def md5(message: bytes) -> bytes:
    ...
    #这一步太妙了，左右移后&0xff就保证该字节有值，而其他字节为零，然后再或，完成高低位对换
    ...
    for i in range(8):
        ascii_list.append((msg_length >> (8 * i)) & 0xff)      #将消息长度变成8个数，存入其中，妙
    ...
    for i in range(len(ascii_list) // 64):
        ...
        for j in range(64):
            # 64轮的主循环
            if 0 <= j <= 15:            #g是规定好的，指定分组的第j个32比特
                f = F(b, c, d) & 0xffffffff     #f是非线性函数的结果
                g = j
                # print("flag1:",g)
            elif 16 <= j <= 31:
            ...
            # 第i个消息块，第g个32-bit
            s = i * 64 + g * 4
            w = W(ascii_list[s], ascii_list[s + 1], ascii_list[s + 2], ascii_list[s + 3])   #得到一个32位无符号数    
            bb = (L((a + f + K[j] + w) & 0xffffffff, R[j]) + b) & 0xffffffff        #步函数结束之后的操作
            a, b, c, d = aa, bb, cc, dd
            # print(b)
        h0 = (h0 + a) & 0xffffffff
        h1 = (h1 + b) & 0xffffffff
        h2 = (h2 + c) & 0xffffffff
        h3 = (h3 + d) & 0xffffffff
    h0, h1, h2, h3 = reverse(h0), reverse(h1), reverse(h2), reverse(h3)     #这里把四个32位拼成128位，涨姿势
...
if __name__ == '__main__':
    ...
    message=input("请输入需要进行md5加密的消息:")
    #---把输入转成字节流---
    #---形如b'abcdefgh'---
    message=message.encode()
    print("解密后的消息是:",md5(message))

    import hashlib
    t=hashlib.md5()
    t.update(message)

    print("md5库进行验证：",t.hexdigest())